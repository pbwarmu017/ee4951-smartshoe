...
void eeprom_writePage(unsigned short writepage_address, unsigned char data[][8])
{
 ...
 if(writepage_address % 0x20 != 0) return;
 I2C_MasterStart();
 I2C_MasterWrite(0b10100000 | (0b000 << 1) | 0x00);
 I2C_MasterWrite(addressmsb);
 I2C_MasterWrite(addresslsb);
    for(char writepage_temprow = 0; writepage_temprow < 2; writepage_temprow++)
    {
        ...
        {
            I2C_MasterWrite(data[writepage_temprow][writepage_tempcol]);
        }
        I2C_MasterStop();
        ACK_Poll();
    }
...
void eeprom_storeBurstGroup(unsigned short burstgroup_address, unsigned short data[][8])
{
 ...
 for(unsigned char pagewritten = 0; pagewritten < 1; pagewritten++)
 {
  ...
  I2C_MasterWrite(addressmsb);
  I2C_MasterWrite(addresslsb);
  for (unsigned char storeburst_temprow = 0; storeburst_temprow < 2; storeburst_temprow++)
  {
            for(unsigned char storeburst_tempcol = 0; storeburst_tempcol < 8; storeburst_tempcol++)
            {
                I2C_MasterWrite((unsigned char)((data[storeburst_temprow + (pagewritten * 2)][storeburst_tempcol]) >> 8));
                I2C_MasterWrite((unsigned char)(data[storeburst_temprow + (pagewritten * 2)][storeburst_tempcol]));
            }
  }
        I2C_MasterStop();
        ACK_Poll();
